...
// will add 2d camera, circles to RenderLayers(1)
// passed in: start_color(h,s,l)
pub fn add_circles_to_render_layer_1(
    mut commands: Commands,
    start_color_res_opt: Option<Res<StartColor>>,
) {
    if let Some(start_color_res) = start_color_res_opt {
        let start_color = *start_color_res;
        println!("StartColor: {}", start_color);
        let first_pass_layer = RenderLayers::layer(1);

        commands.remove_resource::<StartColor>();
        commands
...
pub fn rotate_colors_of_circles(mut query: Query<&mut DrawMode, With<ExampleShape>>) {
